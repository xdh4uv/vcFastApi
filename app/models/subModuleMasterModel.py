...
class SubModuleMaster(Base):
    __tablename__ = "sub_modules_master"
    __table_args__ = {"schema": "modules"}

    sub_module_id= Column(UUID(as_uuid=True), primary_key=True, default=uuid.uuid4)
    sub_module_name = Column(String(255), nullable=False)
    sub_module_description = Column(Text, nullable=True)
